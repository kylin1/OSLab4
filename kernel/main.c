...
void customer_same(int customer_id){
	...
	if(waiting < CHAIR_NUM){
		check_int("customer arrive & waiting. id:",customer_id,ORANGE);
		//等待的顾客占用的椅子数目+1
		waiting ++;
		...
		check_int("get service and leave! id:",customer_id,ORANGE);
	}else{
		//没有空的椅子,人满了,顾客离开
