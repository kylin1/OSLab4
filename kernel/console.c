...
PUBLIC void init_screen(TTY *p_tty) {
    ...
    p_tty->p_console->cache_char[0] = '\0';
...
PUBLIC void clear_console_cache(CONSOLE *p_con) {
    ...
    p_con->cache_char[0] = '\0';

    p_con->cache_char_mem_pos[0] = p_con->original_addr;
...
/*======================================================================*
			   ...
			   向控制台输出带有颜色的字符串
 *======================================================================*/
PUBLIC void out_string(CONSOLE *p_con, char *str, int *color, int debug) {
    int size = str_length(str);
    int i;
    //输出每一个字符
    for (i = 0; i < size; i++) {
        out_char(p_con, str[i], color[i], debug);
    }
}

PUBLIC void out_string_color(CONSOLE *p_con, char *str, int debug) {
    int size = str_length(str);
    int i;
    //输出每一个字符
    for (i = 0; i < size; i++) {
        out_char(p_con, str[i], 1, debug);
    }
}

PUBLIC void out_string_no_color(CONSOLE *p_con, char *str, int debug) {
    int size = str_length(str);
    int i;
    //输出每一个字符
    for (i = 0; i < size; i++) {
        out_char(p_con, str[i], 0, debug);
    }
}
...
PUBLIC void out_char(CONSOLE *p_con, char ch, int color, int debug) {
    //搜索模式 && debug == 0,加入缓存.否则代表调试,不加入缓存
    if (p_con->in_search == 1 && debug == 0) {
    ...
    //显存的起始地址是0XB8000,大小为32KB,范围是0XB8000~0XBFFFF
    //这个内存段中每两个字节代表一个字符,低字节为ASCII码,高字节为属性
    //一个屏幕25行,一行80个字符,大小为25*80*2 = 4000字节,4KB,可以设置8个屏幕

    //显存起始位置 + 当前光标 * 2
    u8 *p_vmem = (u8 *) (V_MEM_BASE + p_con->cursor * 2);

    //这个字符被输出之前光标的位置
    int ch_pos_in_mem = p_con->cursor;
    ...
    switch (ch) {
        ...
        case '\t': {
            ...
            //如果位置处在一行的末尾(77,78,79三个位置),还要让光标跳到下一行的开始位置
        ...
        case '\b':
            ...
            if (p_con->cursor > p_con->original_addr) {
                ...
                int origin_pos = p_con->cache_char_mem_pos[p_con->current_char_pos];
                ...
                p_con->cache_char[p_con->current_char_pos] = '\0';
                p_con->cache_color[p_con->current_char_pos] = 0;
                p_con->cache_char_mem_pos[p_con->current_char_pos] = now_cursor;
            }
            break;
    ...
    //不在搜索模式中,且这个字符不是空格,正常加入缓存,记录这个字符在显存中的位置
    if (p_con->in_search == 0 && debug == 0 && ch != '\b') {
        //将输出的字符加入缓存,加入上一次代表'\0'的地方
        p_con->cache_char[p_con->current_char_pos] = ch;
        p_con->cache_color[p_con->current_char_pos] = 0;
        //保存这个字符被输出之前光标的位置
        p_con->cache_char_mem_pos[p_con->current_char_pos] = ch_pos_in_mem;
        //位置前移
        p_con->current_char_pos++;
        //在新的地方加入结尾字符
        p_con->cache_char[p_con->current_char_pos] = '\0';
        p_con->cache_color[p_con->current_char_pos] = 0;
        p_con->cache_char_mem_pos[p_con->current_char_pos] = (ch_pos_in_mem + 1);
    }
